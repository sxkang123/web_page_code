<template>
    <section>
        ...
        <el-row style="margin-bottom:8px" v-if="!hideTool">
            <el-col :span="24" v-if="!showRight" align="left">
                <el-col :span="18" align="left">
                    ...
                    <div v-if="showCenterInfo" style="display:inline;margin-right:10%;float: left">
                    ...
                    <el-button type="primary" size="small" @click="handlCustomeSearch" v-if="customSearch"
                               icon="search">高级查询
                    </el-button>
                    ...
                    <el-button type="primary" size="small" @click="handleAdd" v-if="!hideAdd">{{addtitle}}</el-button>
                     <el-button type="primary" size="small" @click="handSelfAdd" v-if="showSelectCode">{{addtitle}}</el-button>
                    <el-tooltip class="item" effect="dark" content="导出内容为当前查询条件下所有数据" placement="bottom">
                        <el-button type="primary" @click="handleExport" size="small" v-if="showExportExcel">{{expoertExcelTitle}}</el-button>
                    </el-tooltip>
                   <el-tooltip class="item" effect="dark" content="导出内容为当前查询条件下所有图片" placement="bottom">
                        <el-button type="primary" @click="handleSelfExport" size="small" v-if="showExportImage":disabled="isSelfExport==1">{{expoertImageTitle}}</el-button>
                    </el-tooltip>
                    ...
                    <!--统计页面日期选框-->
                    <div v-if="showanalysisdate" style="float:left;">
                        <el-date-picker
            ...
            <el-col :span="24" v-if="showSelectCode" align="left"style="margin-top:20px">
                <el-col :span="24" align="center">
                    <el-select v-model="blankcode_unionid" filterable placeholder="全部厂商" @change="changeCodeUnion"
                                   style="width:150px;margin-left:20px">
                        <el-option
                                v-for="item in ulist"
                                :label="item.value_name"
                    ...
                    <el-select v-model="blankcode_parkid" filterable placeholder="全部车场" @change="selectCodeInfo"
                                   style="width:150px;margin-left:20px">
                        <el-option
                                v-for="item in plist"
                                :label="item.value_name"
                    ...
                    <el-select v-model="blankcode_state" filterable placeholder="全部状态" @change="selectCodeInfo"
                                   style="width:150px;margin-left:20px">
                        <el-option
                                v-for="item in slist"
                                :label="item.value_name"
        ...
        <el-table :data="table" border highlight-current-row style="width:100%;" :height="tableheight"
			...
			<el-table-column label="操作" :width="btswidth" v-if="!hideOptions" align="center" fixed="left" >
				<template scope="scope">
					<el-button size="small" v-if="!hideEdit" type="text" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
                    ...
                    <el-button v-if="showsetpayexcep" size="small" type="text" @click="openSetPayExcep(scope.$index, scope.row)" :disabled="scope.row.state==3" >处理</el-button>
					...
					<el-button v-if="showMore" size="small" type="text" @click="handleQueryMore(scope.$index, scope.row)">查看</el-button>
                    <el-button v-if="showBack" size="small" type="text" @click="handleShowBack(scope.$index, scope.row)" :disabled="isDistibution==1 || scope.row.state==3 || scope.row.state==4 || (scope.row.trade_no.indexOf('alinosencepay')==-1)">退款</el-button>
                    <el-button v-if="showComponent" size="small" type="text" @click="handleShowComponent(scope.$index, scope.row)">查看进件资料</el-button>
                    <!--
                    <el-button
                        v-for="bt in bts"
                        v-on:click="bt.func"
        ...
        <el-dialog
            ...
            <div class="el-message-box__status el-icon-warning"></div>
            <br/>
            <div style="margin-left:50px;vertical-align:middle;">确定要处理吗？</div>
            <span slot="footer" class="dialog-footer">
				<el-button @click="setpayExcep = false" size="small">取 消</el-button>
				<el-button type="primary" @click="handSetPayExcep" size="small">确 定</el-button>
			</span>
        </el-dialog>
...
<script>
    import {path,blankcodestate} from '../api/api';
    ...
    export default {
        ...
        props:['tableitems','fieldsstr','hideOptions','hideExport','hideAdd','hideSearch','customSearch', 'showRight','showLeftTitle','leftTitle', 'editFormRules','addFormRules',
            'tableheight','bts','btswidth','queryapi','queryparams', 'exportapi','editapi','setpayexcpapi','addapi','delapi', 'searchtitle','addtitle','addfailmsg',
            'dialogsize','showqrurl','showdelete','showmapdialog','showMap','showsetting','hidePagination','showParkInfo','hideTool','showCenterInfo','showanalysisdate',
            'showresetpwd','showdateSelector','hideEdit','showMore','showBack','showComponent','isDistibution','showsetpayexcep','showExportImage','showExportExcel',
            'expoertImageTitle','expoertExcelTitle','showSelectCode'],
        methods: {
            //控制表格样式
            rowstyle(row, index) {
                if (index == 0) {
            ...
            handleSearch() {
                ...
                for (var i = 0; i < this.tableitems.length; i++) {
                    ...
                    } else if (this.tableitems[i].customSelect == 'parkoperator') {
                        //重置该selectlist,根据
                        var params;
                        if (user.roleid == 1) {
            ...
            handlCustomeSearch() {
                ...
                var user = sessionStorage.getItem('user')
                user = JSON.parse(user)
                console.log('handlCustomeSearch');
                for (var i = 0; i < this.tableitems.length; i++) {
                    if (this.tableitems[i].customSelect == 'park') {
                        var params;
                        var params;
                        if (user.roleid == 1) {
                        } else if (user.roleid == 2) {
            ...
            handleEdit(index, row) {
                ...
                console.log(user);
                for (var i = 0; i < this.tableitems.length; i++) {
                    console.log(this.tableitems[i].subs);
                    if (this.tableitems[i].subs[0].customSelect == 'parkserver') {
                        //重置该selectlist,根据
                        var params;
                        if (user.roleid == 1) {
                            if (this.tableitems[i].commonSelect == 'local_all') {
                    ...
                    if (this.tableitems[i].subs[0].customSelect == 'parkoperator') {
                        //重置该selectlist,根据
                        var params;
                        console.log(this.tableitems[i]);
                        if (user.roleid == 1) {
                            if (this.tableitems[i].subs[0].commonSelect == 'local_all') {
                                params = {'com_id': row.id, 'state': 1, 'token': sessionStorage.getItem('token')}
                            } else if (this.tableitems[i].commonSelect == 'all') {
                                params = {'query': 1, 'token': sessionStorage.getItem('token')}
                            }
                        } else if (user.roleid == 2) {
                            if (this.tableitems[i].subs[0].commonSelect == 'local_available') {
                                params = 'state=1&token=' + sessionStorage.getItem('token');
                            }
                        }
                        console.log("getoperator");
                        this.$ajax({
                            url: path + '/getdata/operatorlist' + "?" + params,
                            data: params,
                            //method:'post',
                            async: false,
                            success: function (ret) {
                                vm.tableitems[i].subs[0].selectlist = ret
                            }
                        })
                    }
                }
            ...
            handlesetting(index, row) {
             ...
             //查看更多信息
            handleQueryMore(index, row){
                //调用父组件的方法,传row
                this.$emit('queryMore',row)
            },
             //退款
            handleShowBack(index, row){
                //调用父组件的方法,传row
                this.$emit('queryBack',row)
            },
             handleShowComponent(index, row){
                //调用父组件的方法,传row
                this.$emit('queryComponent',row)
            },
             handSelfAdd(){
                //调用父组件的方法
                this.$emit('selfAdd')
             ...
             handleSelfExport(){
                  //调用父组件的方法
                var params="union_id="+this.blankcode_unionid+"&park_id="+this.blankcode_parkid+"&state="+this.blankcode_state+"&token="+sessionStorage.getItem('token')
            ...
            openSetPayExcep(index, row) {
                this.rowid = row.id;
                this.tradeno = row.trade_no;
                this.setpayExcep = true
            },
            //删除
            handledelete() {
                var vm = this;
            ...
            handSetPayExcep() {
                var vm = this;
                var api = this.setpayexcpapi;
                var qform = this.sform;
                var dform = {'id': this.rowid,'trade_no':this.tradeno,'token': sessionStorage.getItem('token')}
                //发送请求,删除id为row.id的数据
                vm.$post(path + api, dform, function (ret) {
                    if (ret.validate != 'undefined' && ret.validate == '1') {
                        //过期.重新登录
                        ...
                        if (ret.state > 0) {
                            ...
                            vm.setpayExcep = false
                        } else {
                            //更新失败
                            vm.$message({
                                message: ret.errmsg?ret.errmsg:"处理失败",
                                type: 'error',
                                duration: 3200
                            });
                        }
            ...
            changeCodeUnion(){
                var _this = this
                if(_this.blankcode_unionid!=-1){
                    _this.isSelfExport=0
                }
                 this.axios.all([common.getParkList("&union_id="+_this.blankcode_unionid)])
                .then(this.axios.spread(function(park){
                    _this.plist=park.data
                }))
               _this.selectCodeInfo();
            },
            selectCodeInfo(){
                 this.sform={"union_id":this.blankcode_unionid,"park_id":this.blankcode_parkid,"state":this.blankcode_state,"token":sessionStorage.getItem('token')}
                this.getTableData(this.sform);
            }
        },
        mounted() {
        ...
        activated() {
            ...
            this.isSelfExport = 1
            var _this = this
            this.axios.all([common.getUnionList(),common.getParkList()])
			.then(this.axios.spread(function(union,park){
                var urr  =
						{"value_name" : "全部厂商",
							"value_no" :"-1"}
                var prr  ={"value_name" : "全部车场",
							"value_no" :"-1"}  
                 _this.ulist.push(urr);
				_this.plist.push(prr);                
                for(var unionInfo  of union.data){  
                    _this.ulist.push(unionInfo)
                }
                for(var parkInfo  of park.data){  
                    _this.plist.push(parkInfo)
                }
               
			}))
            _this.slist = blankcodestate
        },
    }
