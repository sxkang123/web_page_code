<template>
	<section>
		<common-table 
			...
			v-on:setting="operatorsetting"
			ref="bolinkoperatortable"
		></common-table>
...
<script>
	import { path,stateitems,getUnionList,checkPhone,phonewidth,belongitems,ynitems} from '../../api/api';
	...
	export default {
		...
		data(){
			return {
				...
				addapi:'/operator/add',
				btswidth:'120',
				fieldsstr:'id__union_id__name__phone__area__ctime__utime__state__balance__tid__isdistibution',
				tableitems:[
					{hasSubs:false,subs:[
						{label:'泊链编号',prop:'id',width:'170',type:'number',editable:false,searchable:true,align:'center',},
					]},
					{hasSubs:false,subs:[
						{label:'所属平台',prop:'union_id',width:'280',type:'selection',selectlist:this.ulist,editable:false,searchable:true,addable:true,unsortable:true,
					...
					{hasSubs:false,subs:[
						{label:'余额',prop:'balance',width:'150',type:'number',editable:false,searchable:true,align:'center',
							format:function(row){
								return common.balanceformat(row.balance,2)
							}
						},
					]},
					{hasSubs:false,subs:[
		...
		methods:{
			...
			delCard(){
				var dform = {'id':this.card_id,'platform_id':this.operatorid,'user_type':5,'token':sessionStorage.getItem('token')}
				var vm = this
				//console.log(dform)
			...
			onAddCard(aform){
				...
				this.$extend(aform,{'platform_id':vm.operatorid})
				this.$extend(aform,{'user_type':5})
				//console.log(this)
			...
			getTableData(){
				var vm=this;
				this.loading = true;
				var sform = {'token':sessionStorage.getItem('token'),'platform_id':this.operatorid,'user_type':5}
		...
		activated(){
			window.onresize=()=>{this.tableheight=common.gwh()-135;}
			this.tableheight=common.gwh()-135;
			this.$refs['bolinkoperatortable'].$refs['search'].resetSearch()
			this.$refs['bolinkoperatortable'].getTableData({})
			let _this = this
			//所有厂商,所有运营商
			this.axios.all([common.getUnionList(),common.getBankInfo()])
				.then(this.axios.spread(function(union,bankinfo){
