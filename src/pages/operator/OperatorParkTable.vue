	...
	export default {
		...
		data(){ 
			return {
				...
				btswidth:'70',
				fieldsstr:'id__union_id__operator_id__park_id__name__address__phone__mobile__ctime__utime__total_plot__empty_plot__state__balance',
				tableitems:[
					{hasSubs:false,subs:[
		...
		activated(){
			...
			//请求列表
			//所有厂商,所有运营商
			this.axios.all([common.getOperatorList(),common.getUnionList(),common.getBankInfo(),common.getBaPayUnionList()])
			.then(this.axios.spread(function(operator,union,bankinfo,paylist){
				_this.slist=operator.data
