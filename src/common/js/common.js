...
export default{
	...
	nameformat:function(row,list,col){
		for(var x in list){
			if(row[col]==null||row[col]=='-1'){
				if(col=='com_id'||col=='park_id'|| col=='pay_type' || col=='pay_channel' ||col=='state' ){
					return '-'
				}else if(col=='car_union_id'){
					return '-'
				}else if(col=='server_id'){
					return '(无服务商)'
				}else if(col=='operator_id'){
                    return '(无运营商)'
				}else if(col=='add_account_state'){
                    return '未提交'
				}
			}
			if(list[x].value_no==row[col]){
				return list[x].value_name
			}else if(col=='docking_type'){
				if(row[col]==1){
					return '车场';
				}
    ...
    getOperatorList:function(params){
        ...
        return axios.get(path+'/getdata/operatorlist'+param)
    },
